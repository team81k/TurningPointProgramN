...
void driveTransform(double position, bool wait = true, long timeout = -1)
{
    ...
    if(wait) while(fabs(differentialPID.getError()) > 25 && (timeout == -1 || pros::millis() - waitStart < timeout)) pros::delay(3);
}
...
void driveStraight(double meters, bool wait = true, long timeout = -1)
{
    ...
    driveThreshold = 50;
    long waitStart = pros::millis();
    if(wait) while(fabs(forwardDrivePID.getError()) > driveThreshold && (timeout == -1 || pros::millis() - waitStart < timeout)) pros::delay(3);
}
...
void driveTurn(double radians, bool wait = true, long timeout = -1)
{
    ...
    driveThreshold = 50;
    long waitStart = pros::millis();
    if(wait) while(fabs(turnDrivePID.getError()) > driveThreshold && (timeout == -1 || pros::millis() - waitStart < timeout)) pros::delay(3);
}

void waitOnDrive(long timeout = -1)
{
    long waitStart = pros::millis();
    if(driveType == 1) while((fabs(forwardDrivePID.getError()) || fabs(differentialPID.getError()) > 25) > driveThreshold && (timeout == -1 || pros::millis() - waitStart < timeout)) pros::delay(3);
    else if(driveType == 2) while((fabs(turnDrivePID.getError()) || fabs(differentialPID.getError()) > 25) > driveThreshold && (timeout == -1 || pros::millis() - waitStart < timeout)) pros::delay(3);
}
